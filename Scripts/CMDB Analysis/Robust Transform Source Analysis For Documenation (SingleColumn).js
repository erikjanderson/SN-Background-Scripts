/* 
    Title: Source Analysis Builder For Documentation
...
function getEntityFields(entityId, fieldName, passThroughOps) {
    ...
    while (fieldGr.next()) {
        ...
        getDictionaryInfo(targetField, targetField.table, targetField.name);
...
//Add more dictionary information to a target field
function getDictionaryInfo(targetField, table, element){
    if(gs.nil(table) || gs.name(element)){
        return;
    }
    ...
    if(dictionaryGr.next()){
        targetField.label = dictionaryGr.getValue("column_label");
        targetField.type = dictionaryGr.internal_type.getDisplayValue();
        if(dictionaryGr.getValue("reference")){
            targetField.reference_table = dictionaryGr.reference.name + "";
        }
    }
}
...
function excelReportFields(entityFields) {
    ...
    for (var i = 0; i < entityFields.length; i++) {
        ...
        reportString += '\n\t' + sourceObj.fields + '\t' + sourceObj.sampleValues + '\t' + fieldName + '\t' + (field.reference_table || "")
    }
    return reportString;
}
